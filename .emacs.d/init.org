...
Run the following commands only if running in graphical mode.

#+BEGIN_SRC emacs-lisp
(if (display-graphic-p)
    ;; Only run this command in graphical mode. It is not used at all in
    ;; terminal mode anyways. This line otherwise causes problems in Cygwin
    (tool-bar-mode nil))
#+END_SRC
