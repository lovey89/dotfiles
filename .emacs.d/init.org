...
Run the following commands only if running in graphical mode. If graphical mode
is not installed these commands will cause problems even if you're running in
terminal mode

#+BEGIN_SRC emacs-lisp
(if (display-graphic-p)
    ;; Only run these command in graphical mode
    (scroll-bar-mode nil)     ; Remove the scroll bars
    (tool-bar-mode nil)       ; Remove the toolbar
)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;; In cygwin the end button shows up as <select>
